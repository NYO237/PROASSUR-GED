...
-- Table Fille : Quittance (Peut rester ici)
...
-- Table Fille : Carte Rose (Déplacée vers le haut)
...
-- Table Fille : Attestation (Carte Bleue) (Déplacée vers le haut)
...
-- Table Fille : Contrat (Créée en dernier avec les clés étrangères)
CREATE TABLE contrat (
    ...
    -- Nouvelles colonnes pour les liaisons
    id_carte_rose INT NOT NULL,
    id_attestation INT NOT NULL,
    
    -- Contraintes d'unicité pour garantir le 1 à 1
    CONSTRAINT uq_contrat_carte_rose UNIQUE (id_carte_rose),
    CONSTRAINT uq_contrat_attestation UNIQUE (id_attestation),
    CONSTRAINT uq_bureau_police UNIQUE (code_bureau, num_police),
    
    -- Clés étrangères de l'héritage et des liaisons
    ...
    CONSTRAINT fk_contrat_carte_rose FOREIGN KEY (id_carte_rose) 
        REFERENCES carte_rose(id_document) ON DELETE RESTRICT,
    CONSTRAINT fk_contrat_attestation FOREIGN KEY (id_attestation) 
        REFERENCES attestation(id_document) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
